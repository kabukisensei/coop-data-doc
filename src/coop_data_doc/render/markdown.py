...
def _finding_row_key(finding: ReviewFinding) -> tuple:
    """Deterministic display order: severity rank, then rule, then line."""
    return (
...
def _findings_chip(findings: list[ReviewFinding]) -> str:
    """The one-line advisory chip under a page's H1 (plain markdown)."""
    count = len(findings)
    noun = "review finding" if count == 1 else "review findings"
    return f"> ⚠ **{count} {noun}** — see below."


def _findings_section(findings: list[ReviewFinding]) -> str:
    """The per-object "Review findings" table: Severity | Rule | Message |
    File:line, sorted severity-rank then rule then line. Advisory only —
    findings never affect the build."""
    lines = [
        "## Review findings",
        "",
    ...
    for finding in sorted(findings, key=_finding_row_key):
        lines.append(
            f"| {_cell(finding.severity)} | {_cell(finding.rule_id)} "
            f"| {_cell(finding.message)} | {_cell(finding.location)} |"
        )
    return "\n".join(lines)
...
def render_node_page(
    ...
    *,
    ...
    standalone call) they're derived on demand. ``findings`` (issue #38) are this
    node's joined review findings; ``None``/empty renders exactly today's page.
    """
    ...
    parts = [
        ...
        "",
        # advisory review-findings chip (issue #38) — points at the section below
        *([_findings_chip(findings), ""] if findings else []),
        ...
        *([_upstream_section(graph, node, direct_upstream), ""] if _wants_upstream_tree(graph, node) else []),
        # joined review findings (issue #38) — advisory, renderer-layer only
        *([_findings_section(findings), ""] if findings else []),
    ]
    parts += [
        "## Lineage",
        "",
...
def _index_page(graph: LineageGraph, project_name: str, reviews: ReviewJoin | None = None) -> str:
    ...
    # review-findings roll-up (issue #38) — rendered only when review files
    # were composed into this build; a plain build's index is byte-identical.
    if reviews is not None:
        noun = "review finding" if reviews.total == 1 else "review findings"
        lines.append("")
        lines.append("## Review findings")
        lines.append("")
        lines.append(
            f"[{reviews.total} {noun}](findings.md) composed from the supplied "
            "coop-sql-review / coop-dax-review reports — see the Findings page "
            "for the per-object breakdown. Advisory only; findings never gate a build."
        )
...
def _severity_heading(severity: str) -> str:
    return severity.capitalize() if severity else "(no severity)"


def render_findings_page(graph: LineageGraph, reviews: ReviewJoin, project_name: str) -> str:
    """The top-level Findings page (findings.md): summary counts per tool and
    severity, matched findings grouped severity → rule with links to each
    object's page, the never-dropped unmatched section, and the agent-review
    count note. Deterministic ordering throughout."""
    lines = [f"# Review findings — {project_name}", ""]
    lines.append(
        "Advisory standards findings from the `coop-sql-review` / `coop-dax-review` "
        "linters, composed into the portal from the review files supplied at build "
        "time (`--reviews` / the config's `reviews:` list). Findings never affect "
        "exit codes or the `check` gate — the linters are advisory."
    )
    lines.append("")
    lines.append("## Summary")
    lines.append("")
    if reviews.counts:
        lines.append("| Tool | Severity | Findings |")
        lines.append("| --- | --- | --- |")
        for tool in sorted(reviews.counts):
            by_severity = reviews.counts[tool]
            for severity in sorted(by_severity, key=lambda s: (severity_rank(s), s)):
                lines.append(
                    f"| {_cell(tool)} | {_cell(severity or '(no severity)')} | {by_severity[severity]} |"
                )
    else:
        lines.append(
            "_No review findings were loaded — see the [Diagnostics](diagnostics.md) "
            "page for any unreadable review files._"
        )

    # matched findings, grouped severity -> rule; one row per (object, finding)
    rows: list[tuple[ReviewFinding, Node]] = []
    for node_id in sorted(reviews.by_node):
        node = graph.nodes.get(node_id)
        if node is None:
            continue  # defensive: the join only ever targets graph nodes
        rows.extend((finding, node) for finding in reviews.by_node[node_id])
    severities = sorted({f.severity for f, _n in rows}, key=lambda s: (severity_rank(s), s))
    for severity in severities:
        lines.append("")
        lines.append(f"## {_text(_severity_heading(severity))}")
        sev_rows = [(f, n) for f, n in rows if f.severity == severity]
        for rule in sorted({f.rule_id for f, _n in sev_rows}):
            rule_rows = sorted(
                ((f, n) for f, n in sev_rows if f.rule_id == rule),
                key=lambda fn: (
                    fn[1].qualified_display.lower(),
                    fn[1].id,
                    fn[0].line,
                    fn[0].message,
                    fn[0].file,
            ...
            lines.append("")
            lines.append(f"### {_inline_code(rule or '(no rule id)')} ({len(rule_rows)})")
            lines.append("")
            lines.append("| Object | Message | File:line |")
            lines.append("| --- | --- | --- |")
            for finding, node in rule_rows:
                link = f"{node.node_type.value}/{slug(node.id)}.md"
                lines.append(
                    f"| [{_link_text(node.qualified_display)}]({link}) "
                    f"| {_cell(finding.message)} | {_cell(finding.location)} |"
                )

    # findings that matched nothing are listed, never dropped (hard rule 4)
    if reviews.unmatched:
        lines.append("")
        lines.append("## Not matched to a documented object")
        lines.append("")
        lines.append(
            "_These findings reference objects that are not documented in this "
            "portal (or carry no object reference); they are listed here so "
            "nothing is silently dropped._"
        )
        lines.append("")
        lines.append("| Tool | Severity | Rule | Object | Message | File:line |")
        lines.append("| --- | --- | --- | --- | --- | --- |")
        for f in reviews.unmatched:
            obj = f"{f.model}.{f.object}" if f.model else f.object
            lines.append(
                f"| {_cell(f.tool)} | {_cell(f.severity)} | {_cell(f.rule_id)} "
                f"| {_cell(obj)} | {_cell(f.message)} | {_cell(f.location)} |"
            )

    # agent_review items are counted only (v1 joins `findings` alone)
    if reviews.agent_review_counts:
        lines.append("")
        lines.append("## Agent review")
        lines.append("")
        lines.append(
            "_`agent_review` judgment items are not composed into object pages "
            "(v1 joins `findings` only) — counted here; see each tool's own report._"
        )
        lines.append("")
        for tool in sorted(reviews.agent_review_counts):
            count = reviews.agent_review_counts[tool]
            noun = "item" if count == 1 else "items"
            lines.append(f"- {_cell(tool)}: {count} agent-review {noun}")
    lines.append("")
    return "\n".join(lines)
...
def render_markdown(
    ...
    *,
    ...
    ``reviews`` (issue #38) composes joined review findings into the pages and
    adds the top-level Findings page; ``None`` (no review files supplied)
    renders byte-identically to before the feature existed.
    """
    ...
    if reviews is not None:
        findings_path.write_text(
            render_findings_page(graph, reviews, project_name), encoding="utf-8", newline="\n"
        )
        written.append(findings_path)
    elif findings_path.is_file():
        # findings.md only ever exists because review files were composed; a
        # render without them prunes it (mirrors node-page pruning), so `check`
        # flags review-parity drift instead of a stale page lingering forever.
