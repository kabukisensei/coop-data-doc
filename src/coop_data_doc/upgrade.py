"""Upgrade check and dependency freshness (`coop-data-doc upgrade`).
...
The command never self-updates: it inspects how this copy was installed,
checks PyPI for a newer release, and prints the exact command for the user
to run themselves from a fresh shell. Pure logic (classification, planning)
is separated from the network fetcher (injectable) so tests never need the
network.
"""
...
def manual_upgrade_command(plan: UpgradePlan) -> str:
    """The exact command(s) to run from a fresh terminal to perform the upgrade.

    The tool never self-updates — this is the command the user copy-pastes into
    a normal shell (``python`` rather than the absolute interpreter path, no
    ``-q``). Multi-step methods return newline-separated commands — NOT
    ``&&``-chained, which Windows PowerShell 5.1 (the in-box shell, and the one
    this guidance is often shown in) can't parse.
    """
