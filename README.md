...
| `pipx upgrade coop-data-doc` fails on Windows with `[WinError 32] … being used by another process` (a `PermissionError` or `OSError` naming `coop-data-doc.exe`) | Windows can't replace the tool's launcher while it's running. Close any terminal that has the tool running and run `pipx upgrade coop-data-doc` again from a **fresh** terminal (where the tool isn't running). `coop-data-doc upgrade` itself never hits this — it only checks PyPI and prints the command; it runs no upgrade subprocess of its own. |
